...
def import
 @student = Student.new()
    ...
    @config = Configuration.find_by_config_key('AdmissionNumberAutoIncrement')
    @categories = StudentCategory.active 
    file = params[:student][:excell]
   rowStr = importFile(params[:student][:excell])
   # @student.import(params[:excell])
    flash[:notice] = "File Uploaded Successfully!! row = #{rowStr}"
    redirect_to :controller => "student", :action => "admissionChoice"
end
def addStudent(parameters)
    ...
    @config = Configuration.find_by_config_key('AdmissionNumberAutoIncrement')
    @categories = StudentCategory.active
    logger.info(" Printing parameter #{parameters.inspect}" )
   
      logger.info(" Printing admission Number #{parameters["admission_no"]}" )
          @last_admitted_student = Student.find(:last)
         # parameters[:student][:admission_no] = 121 #@last_admitted_student.admission_no.next
          @student = Student.new(parameters[:student])
         
        logger.info(" Printing student #{@student.inspect}" )
       @status = @student.save
...
def importFile(file)
  ...
  logger.info(" Printing row 1 #{sheet1.row_count}")
   header = sheet1.row(0)
   logger.info(" Printing header #{header}" )
   (1..sheet1.row_count-1).each do |i|
    #row = Hash[[header, sheet1.row(i)].transpose]
    rowAr= sheet1.row(i)
    params = {}
     logger.info(" Printing header.size #{header.size}" )
  header.size.times { |j| params[ header[j] ] = rowAr[j] ||""}
   # row.to_hash.slice(*accessible_attributes)
   logger.info(" Printing NAME #{params["first_name"]}" )
  sthash={}
  sthash["student"]=params
   addStudent(params)
   #@student = Student.new(sthash[:student])
    #param = row.to_hash
    #name=params{:first_name}
     
   # puts row# do something interesting with a row
  end
  #headers=["ejamaatNumber","first_name","middle_name"]
#  CSV.foreach(file.path, headers=>true) do |row|
    #myRow = row.to_hash
  # logger.info(" Printing row #{row[1]}" )
 #  @student = Student.new()
   
  #@student = Hash[*row].slice(*accessible_attributes) 
 # end
  #return rows
